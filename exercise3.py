...
def runExercise3():
    ...
    for i in range(nSim):
        ...
        indexYear = np.where(survProbs <= randU[i])  # we find where the tau falls
    ...
    # Now we evaluate the option value considering the different time to defaults, obtaining one option price for each sim
    ...
    for i in range(len(tauDef)):
        if tauDef[i] == 0:
            partialSum = futureCashFlows[0] * recovery  # defaults between 0 and 1, so we don't receive any "pure coupon"
        else:
    ...
    # Compute discounted payoff, one for each year(one for each fwd start call constituting our Cliquet), including notional
    ...
    for i in range(len(tauDef)):
        ...
        if tauDef[i] == 0:
            partialSum = futureCashFlowsMC[0] * recovery  # defaults between 0 and 1, so we don't receive any "pure coupon"
        else:
